...
public:
    ...
    float transformTobeMapped[6];//初值猜测 initial guess
    ...
    void laserCloudInfoHandler(const lio_sam::cloud_infoConstPtr& msgIn)
    {
        ...
        if (timeLaserCloudInfoLast - timeLastProcessing >= mappingProcessInterval) //当前帧与上一帧的时间间隔大于mappingProcessInterval，进行一次计算
        {
            // std::cout << "Hello" << std::endl;
            timeLastProcessing = timeLaserCloudInfoLast;

            updateInitialGuess();           //设置优化初值，以IMU预测的位姿作为初值
            
            extractSurroundingKeyFrames();  //根据当前位姿，和50m内的所有关键帧位姿，恢复关键帧特征
            
            downsampleCurrentScan();        //对当前特征进行下采样
            
            scan2MapOptimization();         //通过scan to map 求解当前位姿
    ...
    bool detectLoopClosure(int *latestID, int *closestID)
    {
        ...
        kdtreeHistoryKeyPoses->setInputCloud(cloudKeyPoses3D);  //历史关键帧位置信息
        kdtreeHistoryKeyPoses->radiusSearch(cloudKeyPoses3D->back(), historyKeyframeSearchRadius, pointSearchIndLoop, pointSearchSqDisLoop, 0);//在kdtreeHistoryKeyPoses中查找距离当前关键帧距离小于historyKeyframeSearchRadius的关键帧
        
        closestHistoryFrameID = -1;
        for (int i = 0; i < pointSearchIndLoop.size(); ++i)//遍历当前帧附近的历史关键帧
        {
            int id = pointSearchIndLoop[i];
            if (abs(cloudKeyPoses6D->points[id].time - timeLaserCloudInfoLast) > historyKeyframeSearchTimeDiff)//时间大于historyKeyframeSearchTimeDiff，触发回环检测
            {
                closestHistoryFrameID = id;//找到距离当前帧距离最近的历史关键帧
                break;
            }
        }
        ...
        *latestKeyFrameCloud += *transformPointCloud(surfCloudKeyFrames[latestFrameIDLoopCloure],   &cloudKeyPoses6D->points[latestFrameIDLoopCloure]);
        ...
        for (int j = -historyKeyframeSearchNum; j <= historyKeyframeSearchNum; ++j)//向前向后各找25帧历史关键帧
        {
    ...
    void performLoopClosure()
    {
        ...
        *nearHistoryKeyFrameCloud = *cloud_temp;
        ...
        // Align clouds
        icp.setInputSource(latestKeyFrameCloud);        //当前帧
        icp.setInputTarget(nearHistoryKeyFrameCloud);   //历史关键帧
        ...
        if (pubIcpKeyFrames.getNumSubscribers() != 0){
            ...
            publishCloud(&pubIcpKeyFrames, closed_cloud, timeLaserInfoStamp, "odom");//发布当前帧
        }

        // Get pose transformation
        float x, y, z, roll, pitch, yaw;
        Eigen::Affine3f correctionLidarFrame;
        correctionLidarFrame = icp.getFinalTransformation();//得到当前帧与历史帧间的位姿关系
        // transform from world origin to wrong pose
        Eigen::Affine3f tWrong = pclPointToAffine3f(cloudKeyPoses6D->points[latestFrameIDLoopCloure]);//当前帧位姿
        // transform from world origin to corrected pose
        Eigen::Affine3f tCorrect = correctionLidarFrame * tWrong;// pre-multiplying -> successive rotation about a fixed frame 修正后的当前帧位姿
        pcl::getTranslationAndEulerAngles (tCorrect, x, y, z, roll, pitch, yaw);
        gtsam::Pose3 poseFrom = Pose3(Rot3::RzRyRx(roll, pitch, yaw), Point3(x, y, z));//经修正后的当前帧位姿
    ...
    {
        // initialization
        if (cloudKeyPoses3D->points.empty())//根据imu姿态进行初始化
        {
        ...
        if (cloudInfo.odomAvailable == true && cloudInfo.imuPreintegrationResetId == imuPreintegrationResetId)//将imu预测的里程计信息作为优化初值
        {
    ...
    void extractNearby()
    {
        ...
        kdtreeSurroundingKeyPoses->setInputCloud(cloudKeyPoses3D); // create kd-tree, 用于存储之前的关键帧
        kdtreeSurroundingKeyPoses->radiusSearch(cloudKeyPoses3D->back(), (double)surroundingKeyframeSearchRadius, pointSearchInd, pointSearchSqDis);//查找出距离当前关键帧50m范围内的关键帧
        for (int i = 0; i < pointSearchInd.size(); ++i)
        {
            int id = pointSearchInd[i];
            surroundingKeyPoses->push_back(cloudKeyPoses3D->points[id]);//最新关键帧周围50m内的关键帧
        }
    ...
    void extractCloud(pcl::PointCloud<PointType>::Ptr cloudToExtract)//根据关键帧位姿，恢复关键帧特征
    {
    ...
    void cornerOptimization()
    {
        ...
        #pragma omp parallel for num_threads(numberOfCores)
        for (int i = 0; i < laserCloudCornerLastDSNum; i++)//遍历当前帧的所有corner特征
        {
            ...
            kdtreeCornerFromMap->nearestKSearch(pointSel, 5, pointSearchInd, pointSearchSqDis);//在map中搜索距离当前特征距离最近的5个corner特征
    ...
    void surfOptimization()
    {
        ...
        for (int i = 0; i < laserCloudSurfLastDSNum; i++)
        {
            ...
            pointAssociateToMap(&pointOri, &pointSel); 
            kdtreeSurfFromMap->nearestKSearch(pointSel, 5, pointSearchInd, pointSearchSqDis);//在map中搜索距离当前特征距离最近的5个surf特征
    ...
    void scan2MapOptimization()
    {
        ...
        {
            kdtreeCornerFromMap->setInputCloud(laserCloudCornerFromMapDS);//50m范围内的关键帧地图
    ...
    {
        if (saveFrame() == false)   //若当前帧不是关键帧，则return
            return;
    ...
    void publishOdometry()
    {
        ...
        tf::StampedTransform odom_2_baselink = tf::StampedTransform(tCur, timeLaserInfoStamp, "odom", "base_link");
